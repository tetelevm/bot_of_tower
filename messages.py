...
from config import TOWER, CRASH_LENS, BOT_USERNAME, WEDNESDAY_MODE

_last_crash_chars = TOWER[:CRASH_LENS[-1]]
...
MSG_start = (
    ...
    f" <code>/enable{BOT_USERNAME}</code>."
)
...
MSG_help = (
    ...
    f"{'- башня строится только по средам'+chr(10) if WEDNESDAY_MODE else ''}"
    "- только одна башня в день!\n"
    "\n"
    "Чтобы меня использовать, нужно меня добавить в чат, дать права на отправку"
    f" сообщений и вызвать команду <code>/enable{BOT_USERNAME}</code>.\n"
    f"{' Также важно не забыть, что я работаю только' if WEDNESDAY_MODE else ''}"
    f"{' по средам, а в остальное время отдыхаю.' if WEDNESDAY_MODE else ''}\n"
    f"{'В четверг' if WEDNESDAY_MODE else 'Ежедневно'}"
    " в 00:00 я сбрасываю все собранные башни"
    f"{' и отключаюсь до 00:00 среды' if WEDNESDAY_MODE else ', чтобы вы не скучали'}.\n"
    "\n"
